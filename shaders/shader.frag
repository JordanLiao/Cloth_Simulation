...
uniform vec3 LightDirection = normalize(vec3(0, -1, 1));
uniform vec3 LightColor = vec3(1.0f, 1.0f, 1.00);

uniform vec3 LightDirection1 = normalize(vec3(0, -1, -1));
uniform vec3 LightColor1 = vec3(0, 1.0f, 1.0f);
...
void main()
{
	...
	vec3 irradiance = AmbientColor + LightColor * max(0, dot(LightDirection, fragNormal))
        +LightColor1 * max(0, dot(LightDirection1, fragNormal));
